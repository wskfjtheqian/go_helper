...
class GoLangInterface2GrpcIntention : GoBaseIntentionAction() {
    ...
    private fun toReq(text: StringBuilder, method: GoMethodSpec) {
        ...
        for (i in 0 until parameters.size) {
            text.append("\t")
            text.append(Utils.uToLine(toType(parametersTypes[index].type!!)))
            text.append(" ")
        ...
        if (null != result) {
            if (result.type != null) {
                if (result.type!!.text != "error") {
                    text.append("\t")
                    text.append(Utils.uToLine(toType(result.type!!)))
                    text.append(" ")
                    text.append("result")
                    text.append(" = ")
                    text.append(1)
                ...
                for (i in 0 until parametersTypes.size) {
                    if (parametersTypes[i].type!!.text != "error") {
                        text.append("\t")
                        text.append(Utils.uToLine(toType(parametersTypes[i].type!!)))
                        text.append(" ")
